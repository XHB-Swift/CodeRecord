    ...
    open class func fetchCachedFile(with url: String,
        ...
        DispatchQueue.global().async {
            guard let cacheDirPath = documentDirPath else {
                mainBlock(.failure(FoundationError.notFoundDir))
                return
            }
            ...
            guard let remoteUrl = URL(string: url) else {
                mainBlock(.failure(FoundationError(code: 10, description: "Invildate url: \(url)")))
                return
            }
        ...
        URLSession.shared.downloadTask(with: url) { location, response, error in
            ...
            if let happenedError = error {
                ...
                guard let location = location,
                    ...
                    lock.signal()
                    completion?(.failure(FoundationError.needToDebugDetails))
                    return
                }
                let statusCode = httpResponse.statusCode
                if statusCode != 200 {
                    lock.signal()
                    completion?(.failure(FoundationError(code: statusCode, description: "Invalidate HTTP Code")))
                    return
                }
                do {
